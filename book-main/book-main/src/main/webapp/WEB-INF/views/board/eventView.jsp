  ...
  <div id="view" class="inner">
    ...
    <table class="table01">
      ...
      <tbody>
        <tr>
          <td>
            <div class="titBox">${edto.title}</div>
            <div class="flexBox"><span class="dateBox">작성일: ${edto.writeday}</span><span class="viewsBox">조회수: ${edto.readnum}</span></div>
          </td>
        </tr>
        <tr>
          <td>
            <c:if test="${not empty fileList}"> <!-- 첨부파일이 있으면 보여주기 -->
              <div class="fileBox"> <!-- 이미지 외 파일은 목록으로 -->
                <c:forEach items="${fileList}" var="filename">
                  ...
                  <c:if test="${!(ext == 'jpg' || ext == 'jpeg' || ext == 'png' || ext == 'gif')}">
                    <p><a href="/static/temp/event/${filename}" target="_blank">${filename}</a></p>
                  </c:if>
                </c:forEach>
              </div>
              ...
              <div class="imgBox"> <!-- 이미지파일은 화면으로 -->
                <c:forEach items="${fileList}" var="filename">
                  <c:set var="ext" value="${fn:toLowerCase(filename.substring(filename.lastIndexOf('.') + 1))}" />
                  <c:if test="${ext == 'jpg' || ext == 'jpeg' || ext == 'png' || ext == 'gif'}">
                    <img src="/static/temp/event/${filename}" alt="${filename}" />
                  </c:if>
                </c:forEach>
              </div>
            </c:if>
    ...
    <div class="btnWrap">
      <a class="btn01" href="/board/eventList">목록</a>
    </div>
    <!-- 관리자계정일 때 -->
    <c:if test="${userid!=null && userid=='admin'}">
    <div class="btnWrap">
      <a class="btn03" href="/admin/eventModify?id=${edto.id}">수정</a>
